drop database if exists blogtachanka;
...
update agentes set titulo_biografia = '“O tiro perfeito, assim como o resgate perfeito, é uma questão de enquadramento.”' where id = 1;
...
update agentes set titulo_biografia = '“A sobrevivência é uma questão de sincronia. A natureza não dá segundas oportunidades.”' where id = 8;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Não gosto de tirar fotos. A iluminação nunca fica boa.“' where id = 9;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Presta atenção, galera: Ninguém morre hoje. Possíveis perguntas vão ter que esperar.”' where id = 10;
update agentes set biografia = 
'Miles Campbell se formou na Cal State LA com bacharelado em Justiça Criminal e imediatamente entrou no LAPD. Recrutado para a SWAT do LAPD, Campbell se destacou trabalhando com suporte tático e incidentes de alto risco. Sua atuação em entradas dinâmicas e deliberadas fez com que logo se tornasse capitão, ao passo que seu excepcional controle de perímetro garantiu sua primeira comenda.Sempre atento à segurança de sua equipe, Campbell trabalhou incessantemente no aperfeiçoamento de técnicas de defesa, de reforço e de equipamentos. Depois da primeira comenda, ele ainda recebeu várias outras e hoje é um especialista certificado em armas de fogo. Transferido para o FBI, onde treinou com a Polícia Federal Alemã (GSG-9) e atuou como agente de campo, ele posteriormente foi integrado à equipe da SWAT do FBI. Campbell participou de sessões de treinamento intradepartamentais com agências de reforço da lei internacional, incluindo S.A.S e G.I.G.N.'
where id = 10; -- castle

update agentes set titulo_biografia = '"algumas vezes,só se chega até a justiça através da bala.."' where id = 11;
update agentes
set biografia
= 'A infância de Brava foi impactada pela morte precoce de seu primo, um espectador inocente em um assalto que deu errado. O envolvimento criminal no incidente motivou Cardoso a buscar sua graduação em direito. Sua impotência para mudar significativamente o sistema como procuradora criminal a deixou desiludida, levando-a a entrar para o Departamento de Polícia Federal (DPF).Como Agente da Polícia Federal, Cardoso combatia crimes interestaduais e federais, até se tornar Comissária da Polícia Federal. Ela comandava investigações, emitia mandados e, depois de uma recomendação especial do DPF, mudou-se para Brasília para se candidatar ao Comando de Operações Táticas (COT). Após completar as 18 semanas de treinamento de operações aéreas, anfíbias e fluviais, Cardoso entrou para a unidade de intervenções de alto risco do COTA especialização de Cardoso de resolver sequestros de aeronaves civis chamou a atenção da Capitã Yumiko “Hibana” Imagawa, mas foi seu histórico com desempenho excepcional que garantiu seu lugar na Viperstrike.'
where id = 11;-- brava

update agentes set titulo_biografia = '“Nem toda criança escolhe o que quer ser quando crescer”.' where id = 12;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Meu trabalho é mantê-los vivos. Dificultem à vontade.“' where id = 16;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Você diz boné, eu digo botnet.”' where id = 17;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Não me pergunte o que eu tô fazendo no meu celular. Você não quer saber”.' where id = 18;
update agentes set biografia = 
...
update agentes set titulo_biografia = '' where id = 19;
update agentes set biografia = 
''
where id = 19; -- finka

update agentes set titulo_biografia = '“Juntos, podemos atingir até as estrelas no céu.”' where id = 20;
update agentes set biografia = 
...
update agentes set titulo_biografia = '' where id = 25;
update agentes set biografia = 
''
where id = 25;-- glaz

update agentes set titulo_biografia = '“Aniversários, pedidos de casamento… Essas coisas devem ser surpresas. Ninguém quer uma granada na cara.”' where id = 26;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Quem procura uma vida dessas só pode ter uns parafusos a menos na cabeça, mas pra mim não tem coisa melhor”.' where id = 27;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Não dê desculpas por quem você é. Só os fracos de mente vão querer que você se diminua.”' where id = 32;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“A vida é a maior máquina de Rube Goldberg.”' where id = 35;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Me imagine como a sua consciência. Eu vejo tudo e também sou impiedosa”.' where id = 38;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“A aceitação da mediocridade é o primeiro passo rumo ao fracasso.”' where id = 39;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“O coração é um mero motor. Como qualquer máquina, ele pode ser aperfeiçoado.”' where id = 46;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Lá no fundo, eu sabia que tinha que viajar. Agora que já fiz isso, tenho certeza de que não viajei o bastante.”' where id = 47;
update agentes set biografia  = 
'Vinda de uma abastada família marroquina, Sanaa El Maktoub viajou consideravelmente ainda na juventude, passando pela Europa e pelo Norte da África sempre sonhando em desbravar as regiões mais remotas. Alistou-se no serviço militar aos 19 anos de idade e posteriormente chegou ao Groupe d’Intervention de La Gendarmerie Royale (GIGR), após se formar na lendária Fortaleza. Ela serviu quatro turnos junto ao Batalhão de Infantaria de Montanha e participou de exercícios armados e missões conjuntas com parceiros da TSCTP (Trans-Saharan Counterterrorism Partnership). Ela se tornou a única especialista da sua unidade em operações ambientais.Entre seus turnos, ela parte em expedições solo. El Maktoub está entre os poucos exploradores capazes de cruzar o Saara, escalar os Alpes, atravessar florestas tropicais asiáticas, deslocar-se dentro do Círculo Polar Ártico... e voltar para sua unidade em condições operacionais. Sob chuva ou sol, no calor do deserto ou em regiões de clima polar, ela segue seu caminho. É uma especialista em sobrevivência atenta e genial que desenha suas viagens em seu caderno.
'
where id = 47; -- nomad

update agentes set titulo_biografia = '“Se for levar um tiro, você bem que podia fazer isso direito.“' where id = 48;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“As melhores ideias vêm quando me livro das regras e instruções.”' where id = 51;
update agentes set biografia = 
'Nascida na família Janković, donos de lojas de brinquedos pela Europa, Anja viajava frequentemente com seus pais, por isso nunca sentiu estabilidade até ir morar com sua tia na Croácia, aos seis anos. Lá começou seus estudos e aprendeu o ofício familiar de confecção de brinquedos, ao mesmo tempo em que desenvolvia uma obsessão por filmes de ficção científica dos anos 60.Ela seguiu sua paixão por criação e tecnologia e estudou eletromecânica numa escola vocacional e, então, engenharia militar na Universidade de Zagreb, destacando-se em engenharia robótica. Ela se viu isolada em razão de sua maneira pouco ortodoxa e das atitudes dos outros quanto à sua transição, por isso focou no trabalho. Sua perspectiva única e seu conjunto de habilidades criativas a colocaram em evidência e chamaram a atenção de Kali, que estava no país em busca de recrutas. Depois de uma oferta, Janković entrou para a Nighthaven.O sucesso da companhia pode ser amplamente creditado às invenções de Osa e seu trabalho de campo. Sua eficiência e a versatilidade de seu arsenal deram à equipe vantagem em todas as operações registradas, e ao se revelar dedicada e confiável, ela conquistou a direção de P&D da Nighthaven.'
where id = 51; -- osa


update agentes set titulo_biografia = '“As pessoas mentem. Os dados não.”' where id = 52;
update agentes set biografia = 
'Díaz cresceu confortavelmente em um período perigoso para a Colômbia, onde sua família unida a ensinou o valor da verdade. Como a mais velha de quatro filhos, ela sentiu o dever de proteger os outros, fazendo-a entrar na Polícia Nacional. Equilibrando sua paixão por computadores e literatura com sua motivação inabalável, ela sobressaiu em trabalho investigativo. Após receber uma condecoração por revelar corrupção entre seus colegas, ela passou pelo treinamento de operações especiais e conquistou uma vaga na AFEAU.Díaz foi surpreendentemente capaz em balística e combate de proximidade para alguém intelectual, tornando-a uma especialista bem equilibrada. Ela trabalhou em contraespionagem com a Diretoria de Inteligência Nacional e liderou uma iniciativa de parceria com desenvolvedores de tecnologia de Realidade Aumentada para aprimorar a eficiência de sua unidade. Ela simultaneamente teve uma vida secreta online, monitorando corrupção sistêmica através da dark web e, ultimamente, descobrindo transações que ligaram diversos oficiais a redes de tráfico na América do Sul. A repressão que se seguiu a colocou no radar do Agente Santiago “Flores” Lucero, que a recomendou à Agente Tainá “Caveira” Pereira para recrutamento.'
where id = 52; -- solis


update agentes set titulo_biografia = '“Força bruta é só metade do caminho. A vitória é determinada na mente de ume estrategista.”' where id = 53;
update agentes set biografia = 
'Vinde de uma família bem unida, Ngoma pôde seguir seus interesses sem muita resistência. Ile aprendeu tudo o que pôde sobre psicologia e física, enquanto recebia treinamento em música clássica com os dois irmãos menores. Ngoma também aprendeu mecânica básica na oficina do pai, reunindo com sucesso o conjunto de habilidades que aplicaria depois em sua carreira.Ile se sobressaiu em treinamento tático e ciências militares, tornando-se engenheire de combate. Brilhante e imaginative, Ngoma mostrou-se inestimável em qualquer tarefa empreendida. Ile auxiliava os colegas com estratégias e tecnologias para desviar a atenção dos oponentes e romper as defesas inimigas. Sua especialidade em tais táticas, que tinham a reputação de minimizar as baixas em todos os lados sem comprometer os objetivos, rendeu a Ngoma várias medalhas de serviço. Por fim, ile chamou a atenção do Grupo de Forças Especiais da Bélgica, onde ile tem servido desde então.'
where id = 53; -- sens


update agentes set titulo_biografia = '' where id = 54;
update agentes set biografia = 
''
where id = 54; -- tachanka


update agentes set titulo_biografia = '“Abrindo caminho!”' where id = 55;
update agentes set biografia = 
...
update agentes set titulo_biografia = '"Se você não tem a impressão constante de estar sendo observado, é porque não está prestando atenção."' where id = 60;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Aprendizado de máquina e trabalho em equipe têm algo em comum: sempre dê o exemplo.“' where id = 61;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Não se preocupe comigo. Eu nem estou aqui.”' where id = 62;
update agentes set biografia = 
'Muitos registros antigos do Hwa não são claros. Seus pais biológicos continuam desconhecidos. O que se sabe é que ele sofreu a perda do irmão mais velho e do pai na tentativa de chegar à Coreia do Sul. Relatórios escolares nos informam até sua adolescência, quando foi adotado pela família Hwa. Relatórios iniciais apontam sinais de um trauma prematuro – recluso, dificuldade na leitura –, porém, suas notas posteriores mostraram muito foco e um grande avanço.Hwa se interessou por eletrônicos e engenharia elétrica. A fim de usar seus talentos e mostrar orgulho por seu país, ele decidiu se juntar à Marinha da Coreia do Sul. Posteriormente, ganhou uma vaga nos Fuzileiros Navais, com destaque para sistemas de radares e tecnologia em furtividade. Escolhido a dedo pelo respeitado 707º Batalhão de Missões Especiais por suas missões bélicas não convencionais, ele provou ser rápido e letal. Hwa serviu junto com a especialista Grace “Dokkaebi” Nam até seu recrutamento na Rainbow.'
where id = 62; -- vigil

update agentes set titulo_biografia = '“Proteger os outros é um chamado. Se você não sente isso, nem vai.”' where id = 63;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Quanto mais perto você está da morte, mais vibrante a vida se torna.”' where id = 64;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Prepare, execute, desapareça”.' where id = 65;
update agentes set biografia = 
'Embora a CIA e o Third Echelon tenham arquivado a maior parte da infância do Fisher como confidencial, sabe-se que ele nasceu no subúrbio Towson de Baltimore, Maryland, onde cresceu com sua avó paterna, Sarah. Mais tarde, ele se alistou em uma academia militar, onde aprendeu a canalizar sua energia, aprimorar suas habilidades e seguir ordens ou ocultar evidências.Depois da primeira academia, Fisher se alistou diretamente na Academia Naval dos Estados Unidos, onde se formou em ciências políticas e posteriormente buscou treinamento com os SEALs. Enquanto servia no Golfo Pérsico, Bolívia, Colômbia, Senegal e Kosovo, Fisher ficou conhecido por sua capacidade de completar as missões, independentemente das baixas em seu pessoal. Após ser premiado com uma Medalha por Serviço Distinto de Defesa, Fisher se dedicou como instrutor na Base Naval de Little Creek, na Virgínia.Fisher foi recrutado pela CIA, onde ele [REDACTED].'
where id = 65; -- zero

update agentes set titulo_biografia = '"Nunca estou despreparado."' where id = 66;
update agentes set biografia = 
...
update agentes set titulo_biografia = '“Não é sobre onde você vai parar. É sobre como transforma isso no seu lar.”' where id = 67;
update agentes set biografia = 
...
select (pontos_quiz) from dados_usuario join usuario 
	on fkUsuario = usuario.id
		where fkUsuario = 1;
select * from usuario;
select *from agentes;
truncate table dados_usuario;
